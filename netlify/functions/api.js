...
const supabase_1 = require("../../dist/services/supabase");
const bot_messaging_1 = require("../../dist/services/bot-messaging");
...
const handler = async (event, context) => {
    ...
    if (method === 'PATCH' && path.startsWith('/submissions/')) {
        try {
            ...
            if (status === 'Approved' && currentData?.status !== 'Approved' && data.telegram_user_id) {
                (0, bot_messaging_1.sendVerificationMessage)(data.telegram_user_id).catch((err) => {
            ...
            if (status === 'Rejected' && currentData?.status !== 'Rejected' && data.telegram_user_id) {
                (0, bot_messaging_1.sendRejectionMessage)(data.telegram_user_id).catch((err) => {
    ...
    if (method === 'POST' && path.includes('/send-message')) {
        try {
            ...
            const { data: submission, error: fetchError } = await supabase_1.supabase
                .from('submissions')
            ...
            const success = await (0, bot_messaging_1.sendCustomMessage)(submission.telegram_user_id, message.trim());
            if (success) {
                return {
                    statusCode: 200,
                    headers: { 'Content-Type': 'application/json' },
